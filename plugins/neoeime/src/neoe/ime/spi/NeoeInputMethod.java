...
public class NeoeInputMethod implements InputMethod {
	...
	public void dispatchEvent(AWTEvent event) {
		...
		if (eventType == KeyEvent.KEY_RELEASED) {
			...
			if (keyCode == KeyEvent.VK_BACK_SPACE && active
					&& imein.length() > 0) {
				e.consume();
			}
	...
	private void initLookupWindow() {
		try {
			lookupWindow = (Window) Const.createInputMethodJFrameMethod.invoke(
					context, new Object[] { "neoeime", Boolean.FALSE });
